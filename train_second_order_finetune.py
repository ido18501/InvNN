...
from training.second_order_finetune_trainer import (
...
def parse_int_list_maybe(value):
    ...
    raise TypeError(f'Cannot parse integer list from value of type {type(value)!r}')
...
def load_pretrained_config(pretrained_model_path: str | Path) -> tuple[dict, Path]:
    ...
    if missing:
        raise KeyError(f'Pretrained config is missing required fields: {missing}')
...
def build_loader(bank_path: str, family: str, batch_size: int, num_workers: int, shuffle: bool) -> DataLoader:
    dataset = FullCurveBankDataset(bank_path=bank_path, family=family)
    return DataLoader(
        dataset,
...
def parse_args():
    ...
    p.add_argument('--lambda2-vec', type=float, default=1.0)
    p.add_argument('--lambda1-cos', type=float, default=0.05)
    p.add_argument('--lambda2-cos', type=float, default=0.05)
    p.add_argument('--lambda2-log', type=float, default=0.02)
    p.add_argument('--lambda-rowsum', type=float, default=1e-3)
...
def main():
    ...
    trainer = SecondOrderFineTuneTrainer(
        ...
        eval_batch_size_curves=args.curve_batch_size,
    )
